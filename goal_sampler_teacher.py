...
class GoalSamplerTeacher:
    def __init__(self, args):
        ...
        self.discovered_goals = generate_simple_goals_demonstrator()
        self.discovered_goals_str = []
        for g in self.discovered_goals:
            goal_str = '['
            for ind, predicate in enumerate(g):
                if predicate == 1:
                    if ind == 0:
                        goal_str += ' 1.'
                    else:
                    ...
                    if ind == 0:
                        ...
                        goal_str += ' -1.'
            goal_str += ']'
            self.discovered_goals_str.append(goal_str)
        #self.discovered_goals_str = [str(g).replace('0', '').replace(',', '') for g in self.discovered_goals]
        self.rank = MPI.COMM_WORLD.Get_rank()
        self.all_achievable_goals = generate_simple_goals_demonstrator()
