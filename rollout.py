...
class RolloutWorker:
    def __init__(self, env, policy, goal_sampler, args):
        ...
        self.teacher_mode = args.teacher_action_mode
    ...
    def generate_proba_from_demo(self, ep, goals_for_proba_prediction=None):
        ...
        for j,goal in enumerate(goals_for_proba_prediction):
            goals = np.array([goal for _ in range(len(ep['act']))]) # duplicate along trajectory the goal for which we compute proba
            action_proba = self.policy.get_action_proba(torch.tensor(ep['act']), ep['obs'][:-1], ep['ag'][:-1], goals, masks_for_proba_prediction[j])
            proba = 1
