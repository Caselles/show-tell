...
class her_sampler:
    def __init__(self, args, reward_func=None):
        ...
        self.natural_demos_vs_exp = False 
    ...
    def sample_her_transitions(self, episode_batch, batch_size_in_transitions, normalization=False, bc=False, sqil=False, sil=False):
        ...
        if self.learner_from_demos and not normalization:
            ...
            percentage = len(demo_indexes)/len(episode_batch['is_demonstration'])

            '''rank = MPI.COMM_WORLD.Get_rank()
            all_percentage = MPI.COMM_WORLD.gather(percentage, root=0)
            if rank == 0: print(np.mean(np.array(all_percentage)), 'Average percentage of demonstrations in HER')'''
            ...
            if len(demo_indexes) == 0:
                ...
                if self.natural_demos_vs_exp:
                    percentage = 1.0
                    demo_count = int(percentage * batch_size)
                else:
                    demo_count = int(self.demos_vs_exp * batch_size)

            '''rank = MPI.COMM_WORLD.Get_rank()
            all_demo_count = MPI.COMM_WORLD.gather(demo_count, root=0)
            if rank == 0: print(np.mean(np.array(all_demo_count)), 'Average demo count')'''
