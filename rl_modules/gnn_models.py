...
class GnnActor(nn.Module):
    ...
    def sample(self, obs, edge_features):
        ...
        action_proba = [x if x<0.5 else 1-x for x in normal.cdf(x_t)[0]]
        #print(np.mean(action_proba), 'TOCHECK')
        y_t = torch.tanh(x_t)
        action = y_t
    ...
    def get_action_proba(self, action, obs, edge_features, cuda=False):
        mean, log_std = self.forward(obs, edge_features)
        std = log_std.exp()
        normals = [Normal(m, s) for m,s in zip(mean, std)]
        #print(action, 'action')
        if cuda:
            action_inverse_tanh = torch.tensor(np.arctanh(action.cpu().numpy())).cuda()
            action_proba_aux = [normal.cdf(action_inverse_tanh[i].clone().detach()).cpu() for i,normal in enumerate(normals)]
        else:
            action_inverse_tanh = np.arctanh(action)
            action_proba_aux = [normal.cdf(action_inverse_tanh[i]) for i,normal in enumerate(normals)]

        action_proba = []
        for p in action_proba_aux:
            action_proba.append([x if x<0.5 else (1-x) for x in p])

        return np.mean(action_proba, axis=1)
...
class GnnSemantic:
    ...
    def get_action_proba(self, action, obs, ag, g, cuda=False):
        edge_features = self.critic.message_passing(obs, ag, g)
        action_proba = self.actor.get_action_proba(action, obs, edge_features, cuda)
        return action_proba
